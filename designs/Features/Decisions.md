...
- If the mid-turn choice isn't made (either because there is no mid-turn switch function or the function doesn't make a choice), then the simulation does not continue.
...
- These checks will only run in debug or test builds and the function will always return `true` otherwise.
