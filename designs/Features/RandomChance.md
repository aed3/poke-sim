...
> This is by no means an amazing or permanent solution. The goal here is to make a passible attempt that can be substituted with better solutions without changing the entire codebase.
...
## Special Notes
Metronome (Move) and Psywave will only be handled when in Fixed Paths mode. Otherwise, Psywave will be set to the average damage output and Metronome will default to Splash (an equally effective move in my opinion).
