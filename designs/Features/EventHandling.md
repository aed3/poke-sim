...
A handler has a dynamic priority if any call to it might affect multiple Pokemon at once and the order in which they are affected matters. This applies to most multi-target moves and residual effects, thus are more prevalent in double battles.
...
- Those 4 calls will distinguish themselves by having one of an `Order_#` tag (where "#" is replaced with 1-4) that's assigned to each active Pokemon whenever the speed order may change:
  ...
  - Switches
  - Tailwind, Trick Room, Rain with a Swift Swimmer active, etc starts or ends.
